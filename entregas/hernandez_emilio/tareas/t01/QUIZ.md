...
Un atributo guarda el estado persistente del objeto en la clase, mientras que una variable local solo vive dentro del método donde se declara. En Bicicleta el atributo es `marca` y la variable local `incrementoKm` en `acelerar()`; en Taqueria el atributo es `nombre` y la variable local `subtotal` en `calcularTotal()`; en Lavadora el atributo es `capacidadKg` y la variable local `tiempoRestante` en `iniciarCiclo()`.
